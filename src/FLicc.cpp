...
// ---------------------------------------------------------
template<class Type>
vector<Type> calc_sel(const vector<Type>& L, int sel_type, const vector<Type>& pars)
{
  int n = L.size();
  vector<Type> sel(n);
  ...
  if(sel_type == 1) {
    Type L50   = pars(0);
    Type steep = exp(pars(1));
    
    for(int i = 0; i < n; i++) {
      sel(i) = Type(1) / (Type(1) + exp(-steep * (L(i) - L50)));
    }
...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_SCALAR(ref_length);     // reference length for inverse-length M
  DATA_INTEGER(M_model);       // 1 = constant M, 2 = inverse-length M
  DATA_SCALAR(a_wl);
  DATA_SCALAR(b_wl);
  DATA_SCALAR(L50_mat);
  ...
  for(int g = 0; g < ngear; g++) {
    ...
    Sel.col(g) = calc_sel(Lmid, sel_type(g), pars);
  }
  ...
  for(int l = 0; l < nlen; l++) {
    if(M_model == 1) {
      ...
      M(l) = Mk * ref_length / Lmid(l);
    } else {
      error("Unknown M_model");
    }
  }
  
  // Maturity and weight at length
  vector<Type> mat(nlen);
  vector<Type> wt(nlen);
  Type mat_steep = Type(1);
  
  for(int l = 0; l < nlen; l++) {
    mat(l) = Type(1) / (Type(1) + exp(-mat_steep * (Lmid(l) - L50_mat)));
    wt(l)  = a_wl * pow(Lmid(l), b_wl);
  }
  ...
  for(int g = 0; g < ngear; g++) {
    for(int l = 0; l < nlen; l++) {
      if(pred_total > Type(0)) {
        mu(l, g) = obs_total * Cpred(l, g) / pred_total + Type(1e-12);
      } else {
        mu(l, g) = Type(1e-12);
      }
    }
  ...
  Type catch_sd = Type(0.05);  // smaller = stronger anchoring
